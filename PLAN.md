...
**Definition of done.**
- [ ] On a fresh CachyOS KDE install with `cargo install superpanels`, a panorama spans three monitors with bezel correction on first try.
- [ ] `superpanels detect` prints the monitor table and exits zero.
- [ ] All bezel-math edge cases listed in SPEC §4.5 have passing tests.
- [ ] `cargo clippy --all-features -- -D warnings` is clean.
- [ ] `cargo test` runs in < 30 s.
...
- [ ] `.gitignore` (target/, *.bak, dist/, ui/node_modules/, ui/dist/).
- [ ] `LICENSE` (MIT or Apache-2.0/MIT dual; pick early).
- [ ] Minimal `README.md` with one-line description + "still in development" badge.
...
- [ ] Mixed orientation (one portrait monitor) — rotation handling.
- [ ] 2×2 grid — vertical bezel logic.
- [ ] Single-monitor degenerate case.
...
- [ ] `load(path) -> Result<DynamicImage>` with friendly error on unsupported format / bad file.
- [ ] `scale_to_fit(img, target, FitMode) -> DynamicImage`. Lanczos3 default.
- [ ] `crop(img, rect) -> DynamicImage` with bounds-check error.
- [ ] `rotate(img, Rotation) -> DynamicImage` (uses `image::imageops::rotate90` etc.).
- [ ] `save_temp(img, name) -> Result<PathBuf>` writing to `$XDG_CACHE_HOME/superpanels/temp/`.
- [ ] Clear temp dir at the start of each run (atomic: remove + recreate).
- [ ] Memory cap: refuse to decode if estimated decoded size > configured limit (default 512 MB).
- [ ] Integration test: load a real test image, run a full `compute_crop_specs → crop → save_temp` pipeline, verify output dimensions and existence.
...
- [ ] Manual smoke test on KDE: does the wallpaper actually change? Does it survive a logout/login? Is it bezel-correct?
...
- [ ] No I/O in tests; use `tempfile`.

### 1.7 CLI wiring (`superpanels-cli/src/main.rs`)
- [ ] `clap::Parser`-derived command tree.
- [ ] `superpanels set <IMAGE> [...flags]` per SPEC §11.1, except `--save-as` (Phase 2).
- [ ] `superpanels detect`.
- [ ] `superpanels config` — print resolved config (debug aid).
- [ ] `--dry-run`: print computed crop specs as JSON, skip file write and backend call.
- [ ] `--verbose` / `-v` increases tracing level; `--quiet` suppresses non-error output.
- [ ] Exit codes per SPEC §11.6.
- [ ] `--no-daemon` flag accepted (no-op in Phase 1; daemon arrives in Phase 2).

### 1.8 Documentation (Phase 1 slice)
- [ ] README: install (cargo), one-line example, link to SPEC.
- [ ] `docs/architecture.md`: a one-page summary of the layout from SPEC §5 with current crate graph.
