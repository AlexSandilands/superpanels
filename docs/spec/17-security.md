...
- **Tauri v2 hardening.** The defaults are not safe enough for an app that touches user-supplied paths and runs subprocesses. We lock down four surfaces:
  ...
  - **No `asset:` protocol for arbitrary paths.** Library thumbnails go through `library_thumbnail`, which is constrained to configured roots. Selected/dropped source previews go through `source_thumbnail`, which returns resized image bytes only after image decode/memory-budget validation. Its trust boundary is *anything reachable through a saved profile body*, not just live picks — a profile's `SpanSource::Single { path }` can carry any absolute path, so the only sandbox on this command is the canonicalize + decode-budget guard. The frontend wraps both as `blob:`/`data:` URLs. This means a webview script can't read arbitrary files via `asset://` even if CSP is bypassed.
