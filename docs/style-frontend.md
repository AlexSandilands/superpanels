...
├── profileSwatch.ts                    ← deterministic colour helper
├── components/
...
└── lib/
    ...
    ├── slideshowController.svelte.ts   ← slideshow state + transport
    ├── canvas/
    │   ├── drag.svelte.ts              ← drag state machine
    │   ├── select.ts                   ← selection-scoped mutations
    │   ├── transformActions.ts         ← gap/cover/reset orchestrations
    │   ├── previewLayout.ts            ← orchestrator + geometry
    │   └── previewLayout/
    │       ├── projection.ts           ← positions + cover-fit rect
    │       └── gaps.ts                 ← neighbour detection +
    │                                      normalisation
    ├── events/window.ts                ← Tauri tray + drag-drop wiring
    ├── library/                        ← image-cache + thumb_cache
    ├── stores/                         ← module-scoped state stores
    │   ├── canvasView.svelte.ts        (zoom, pan, hover, overrides)
    │   ├── imageTransform.svelte.ts    (image rect + source effect)
...
| TS modules              | `camelCase.ts`              | `previewLayout.ts`, `sourceImage.ts`, `profileSwatch.ts` |
| Stores                  | `camelCase.svelte.ts`       | `canvasView.svelte.ts`                                   |
...
`thumb_cache.ts` is a remaining snake_case holdout — see `docs/followups.md`. New TS files use camelCase.
