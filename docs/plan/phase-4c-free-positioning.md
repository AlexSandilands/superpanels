...
**Definition of done.**
- [ ] Image is visible across the full canvas (not only inside monitor cutouts). Monitors render as bordered overlays on top.
- [ ] Drag-to-pan works at ≥ 60 fps and Apply succeeds for *any* offset, including offsets that put part of the source rectangle off-image.
- [ ] Drag-corner-to-resize works; aspect-ratio lock + free modes both supported.
- [ ] Per-monitor regions not covered by the source rectangle are written as black pixels by the apply pipeline (no `LayoutError`, no daemon toast).
- [ ] Mixed portrait + landscape: the user can scale the source so it covers both orientations consistently, and the visual preview matches the applied wallpaper.
- [ ] Old span profiles (no `image_size_mm` in TOML) load with the legacy "auto-fit per FitMode" behaviour preserved — no migration required.
...
- [ ] Add the field with `#[serde(default, skip_serializing_if = ...)]` so existing TOML round-trips unchanged.
- [ ] `cargo test -p superpanels-core` covers: `None` round-trips through TOML; `Some([w, h])` round-trips; an existing `[[profile]]` block without the field deserialises with `image_size_px: None`.
- [ ] Mirror the field in `ui/src/lib/types/profile.ts`. Frontend treats `null` as "use FitMode" and a tuple as "explicit transform."
- [ ] `ui/src/lib/stores/profile.svelte.ts::patchDraft` is unchanged structurally — the new field is just another mutable slot.
...
- [ ] Change the return type from `Result<Rect, LayoutError>` to `Result<CropSliceSpec, LayoutError>` where:
  ```rust
  ...
  ```
- [ ] `CropSpec` (the existing top-level type) embeds `CropSliceSpec` so the apply pipeline knows where to paint and what to leave black. Existing fields `monitor_id`, `rotation`, `fit` stay.
- [ ] `compute_crop_specs_with_offset` honours `image_size_px`: when `Some`, the source rectangle on the canvas is `(offset.x, offset.y, image_size_px.0, image_size_px.1)` regardless of FitMode; when `None`, the existing FitMode-driven placement is used.
- [ ] **Tests** (proptest where useful):
...
- [ ] Add `superpanels_core::image::compose_on_black(slice: DynamicImage, dst_size: (u32, u32), dst_offset: (u32, u32)) -> DynamicImage` (or extend `crop`/`scale_to_fit` rather than introducing a new function — pick whichever keeps `crates/superpanels-core/src/image.rs` under the line cap).
- [ ] Wire it into both apply paths between `crop` and `rotate` so the temp file is always full monitor resolution post-rotation.
- [ ] Test: an apply with a deliberately small `image_size_px` produces a temp file with black borders around the actual image content; size matches monitor `dst_size`.
- [ ] Test: existing offset-zero / FitMode::Fill apply produces identical bytes to today's path (no spurious recompositing).
...
- [ ] Update `draw.ts` to the new layer order. Keep the `serialiseLayout` snapshot helper unchanged so 4d's regression test still works.
- [ ] Update `ui/src/components/MonitorPopout.svelte`'s slice math: the source rectangle is now `(offset, image_size_px)` directly instead of `placeImage(fit, ...)`. Simpler.
- [ ] Test: a saved `image_size_px = None` profile renders identically to today (visual regression via the 4d snapshot harness).
...
- [ ] Four 12×12 px corner handles drawn at the image rect corners. Cursor style per corner (`nwse-resize`, `nesw-resize`).
- [ ] Drag from a corner mutates `image_size_px` and (when the user holds Shift, or by default — pick one) preserves the source aspect ratio.
- [ ] Free-resize mode (no aspect lock) for users who want to stretch — surfaces as a small "lock aspect" toggle next to the FitMode buttons. Default is locked.
- [ ] Snap-to-cover button: a single "Cover all monitors" action (next to the fit buttons) that sets `offset` and `image_size_px` so the smallest scale that fully covers the union of monitor rects is applied. Saves the user from manually scaling for the portrait+landscape case.
- [ ] `R` key behaviour: today resets offset to `[0, 0]`. New behaviour: resets *both* `offset = [0, 0]` and `image_size_px = None` (back to FitMode-driven default).
- [ ] Pointer math respects the `offsetScale` factor (`mmToPx / coreMmToPx`) so the persisted core-canvas-pixel values match what the Rust crop algorithm expects.
- [ ] Drag commit batches `offset` + `image_size_px` into a single `patchDraft` call so the editor's `unsaved` flag fires once per gesture, not twice.
...
- [ ] In `ProfileEditor.svelte`, when `body.type === 'span'` and `body.image_size_px !== null`, show a small read-only panel with the current `offset` (px) and `image_size_px` (px). A "Reset transform" button clears both back to FitMode behaviour.
- [ ] FitMode picker is greyed out (with a tooltip) when `image_size_px` is set, since the explicit transform supersedes it. Picking a FitMode while greyed clears `image_size_px` and re-derives.
- [ ] Test: snapshotting a profile through Save → Reload via `profileStore.refresh()` round-trips both fields losslessly.

## 4c.7 Off-monitor dimming (visual cue)

To make it obvious which part of the image is going to land on the wall:

- [ ] In `draw.ts` step 2 (image rectangle), draw the off-monitor portion of the image at reduced alpha (e.g. 0.4) and the on-monitor portion at full alpha. Implementation: draw at full alpha into a temp canvas, then alpha-fade the regions outside `layout.monitors`. Or draw twice — once at 0.4 across the whole image bbox, once at 1.0 with monitor-rect clip.
- [ ] Toggle (`D` key, or a small icon button in the canvas chrome) to disable the dimming if it's distracting.
- [ ] Respect `prefers-reduced-motion` only insofar as no animation is involved — the dim is a static effect, fine to keep on.

## 4c.8 SPEC updates

The new model needs to be documented before code lands so the spec stays the source of truth:

- [ ] Update `docs/spec/12-gui.md` §12.3:
...
- [ ] Update `docs/spec/04-bezel-math.md` §4.6 ("What the math deliberately does *not* do"):
...
- [ ] Update `docs/spec/08-image-processing.md` to reference the `compose_on_black` step (or whatever shape §4c.3 lands on) in the apply pipeline.

## 4c.9 Migration / compatibility

- [ ] Existing TOML profiles without `image_size_px` continue to apply correctly — verified by an integration test that loads a Phase 4a config and runs `apply_profile`.
- [ ] `superpanels set` CLI: no behaviour change. The CLI doesn't expose `image_size_px` because positioning is a GUI affordance; CLI users get the FitMode-driven default.
- [ ] The new `CropSliceSpec` shape is internal; `preview_crop`'s IPC return type still serialises a `Vec<CropSpec>`, with `CropSpec` extended (additive fields).
...
- [ ] Canvas redraw must still hit the SPEC §19 budget (< 8 ms / ≥ 120 fps median) with the extra image draw + dim layer. The `localStorage.superpanels.bench` hook from Phase 4a still works; re-capture the median + p95 after §4c.4 lands.
- [ ] Apply path: measure end-to-end with letterboxing on a 3-monitor config (one zoomed-out so two monitors are partially uncovered). Should stay under SPEC §19's apply budget. If `compose_on_black` is the bottleneck, the optimisation is to fuse it with the existing `scale_to_fit` step rather than copy-once-pad-once.
